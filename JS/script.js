...
/**
 * Element tit Effect    [  very important study well  ]
 */
...
const hoverdElement =[...document.querySelectorAll("button"),...document.querySelectorAll("a")];;
...
window.addEventListener("mousemove", function (event) {
  cursor.forEach(function (element) {
    ...
    // Cursor dot Postion
    cursor[0].style.left = `${posX}px`;
    cursor[0].style.top = `${posY}px`;

    // Cursor Outline Postion
    setTimeout(function () {
      cursor[1].style.left = `${posX}px`;
      cursor[1].style.top = `${posY}px`;
    }, 100);
    
  });
});

//add hoverd Class when mouse hover 

addEventOnElements(hoverdElement, "mouseover", function () {
  for (let i = 0, len = cursor.length;i <len ; i++) {
    cursor[i].classList.add("hovered");
  }
});
//add hoverd Class when mouse out
addEventOnElements(hoverdElement, "mouseout", function () {
  for (let i = 0, len = cursor.length;i <len ; i++) {
    cursor[i].classList.remove("hovered");
  }
});
